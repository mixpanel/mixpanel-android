...
public class MixpanelPushNotification {
    ...
    public Notification createNotification(Intent inboundIntent) {
        ...
        maybeSetExpandableImage(data);
        maybeSetIconImage(data);
        maybeSetBadgeCount(data);
    ...
    protected void maybeSetBadgeCount(NotificationData data) {
    ...
    protected void maybeSetExpandableImage(NotificationData data) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN && null != data.imgUrl) {
            try {
                Bitmap imageBitmap = getBitmapFromUrl(data.imgUrl);
    ...
    protected Bitmap getBitmapFromUrl(String url) throws ImageStore.CantGetImageException {
    ...
    protected void maybeSetIconImage(NotificationData data) {
        if (data.largeIcon == NotificationData.NOT_SET && Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN && null != data.iconUrl) {
            ImageStore is = new ImageStore(context, "Push Notifications");
            try {
    ...
    /* package */ NotificationData readInboundIntent(Intent inboundIntent) {
        ...
        return new NotificationData(notificationIcon, largeNotificationIcon, whiteNotificationIcon, expandableImageURL, largeIconName, notificationTitle, notificationSubTitle, message, notificationIntent, color, buttons, badgeCount);
    }
    ...
    protected static class NotificationData {
        protected NotificationData(int anIcon, int aLargeIcon, int aWhiteIcon, String aImgUrl, String aIconUrl, CharSequence aTitle, CharSequence aSubTitle, String aMessage, Intent anIntent, int aColor, List<NotificationButtonData> aButtons, int notificationBadgeCount) {
            icon = anIcon;
