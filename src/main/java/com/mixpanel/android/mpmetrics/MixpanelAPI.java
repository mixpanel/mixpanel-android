...
public class MixpanelAPI {
    ...
    MixpanelAPI(Context context, Future<SharedPreferences> referrerPreferences, String token, MPConfig config, boolean optOutTrackingDefault, JSONObject superProperties) {
        ...
        if (!mPersistentIdentity.isFirstIntegration(mToken)) {
            try {
                sendHttpEvent("Integration", "85053bf24bba75239b16a601d9387e17", token, null, lib, libVersion);
        ...
        if (((getContext().getApplicationInfo().flags & ApplicationInfo.FLAG_DEBUGGABLE) != 0)) {
            try {
                if (mToken.length() == 32) { // Only track DevX when the token is valid
                    trackMixpanelDevX(lib, libVersion);
                }
            } catch (JSONException e) {}
        }
    ...
    private void trackMixpanelDevX(String lib, String libVersion) throws JSONException {
        trackDebugLaunch(lib, libVersion);
        promptDevXSurvey(lib, libVersion);
        trackMixpanelImplemented(lib, libVersion);
    }

    private void trackDebugLaunch(String lib, String libVersion) throws JSONException {
        ...
        sendHttpEvent("SDK Debug Launch", "metrics-1", mToken, debugProps, lib, libVersion);
    }

    private void promptDevXSurvey(String lib, String libVersion) throws JSONException {
        ...
        if ((nowMillis - mpSurveyShownDate) > oneDayInMillis) {
            ...
            sendHttpEvent("Dev NPS Survey Logged", "metrics-1", mToken, surveyProps, lib, libVersion);
            mPersistentIdentity.setMPSurveyShownDate(mToken, nowMillis);
        }
    }

    private void trackMixpanelImplemented(String lib, String libVersion) throws JSONException {
        ...
        if (implementationScore >= 3) {
            sendHttpEvent("SDK Implemented", "metrics-1", mToken, implementedProps, lib, libVersion);
            mPersistentIdentity.setHasImplemented(mToken);
        }
    }

    private void sendHttpEvent(String eventName, String token, String distinctId, JSONObject properties, String lib, String libVersion) throws JSONException {
        final JSONObject messageProps = new JSONObject();
    ...
    public void setEnableLogging(boolean enableLogging) {
        ...
        if (((getContext().getApplicationInfo().flags & ApplicationInfo.FLAG_DEBUGGABLE) != 0)) {
            try {
                ...
                sendHttpEvent("Toggle SDK Logging", "metrics-1", mToken, debugProps, null, null);
            } catch (JSONException e) {
                e.printStackTrace();
            }
        }
    ...
    private class PeopleImpl implements People {
        ...
        private JSONObject stdPeopleMessage(String actionType, Object properties)
            ...
            if (((getContext().getApplicationInfo().flags & ApplicationInfo.FLAG_DEBUGGABLE) != 0)) {
                if (!((JSONObject) properties).keys().next().startsWith("$ae_")) {
