...
public class HttpService implements RemoteService {
  ...
  private String getUrlWithFailoverHost(String originalUrl) {
    if (failoverBaseUrl == null || failoverBaseUrl.trim().isEmpty()) {
      return originalUrl;
    }
    try {
      URL original = new URL(originalUrl);
      URL failover = new URL(failoverBaseUrl);

      // Replace host and port (if any) from the failover URL
      String newUrl =
          originalUrl.replace(
              original.getProtocol() + "://" + original.getAuthority(),
              failover.getProtocol() + "://" + failover.getAuthority());

      MPLog.v(LOGTAG, "Switching to failover URL: " + newUrl);
      return newUrl;
    } catch (Exception e) {
      MPLog.e(LOGTAG, "Failed to construct failover URL, using original", e);
      return originalUrl;
    }
  }
