...
public class FeatureFlagManagerTest {
  ...
  /**
   * Waits for flags to become ready by flushing loopers.
   */
  private void waitForFlagsReady() {
    for (int i = 0; i < 30; i++) {
      if (mFeatureFlagManager.areFlagsReady()) {
        // Flush once more to process any pending callbacks
        flushAllLoopers();
        return;
      }
      flushAllLoopers();
  ...
  public void testLoadFlags_whenEnabled_andFetchSucceeds_flagsBecomeReady()
    ...
    waitForFlagsReady();
    assertTrue("Flags should become ready after successful fetch", mFeatureFlagManager.areFlagsReady());
  ...
  @Test
  @Ignore("TODO: requires emulator — async callback timing differs under Robolectric")
  ...
  public void testTracking_getVariantSync_calledOnce() throws InterruptedException, JSONException {
    ...
    // Flush loopers to process the tracking callback posted to main thread
    flushAllLoopers();
  ...
  public void testConcurrentLoadFlagsCalls() throws InterruptedException {
    ...
    // Flush loopers to process all handler messages
    flushAllLoopers();
  ...
  public void testConcurrentGetVariantCalls_whenFlagsNotReady() throws InterruptedException {
    ...
    final CountDownLatch completionLatch = new CountDownLatch(threadCount);
    final List<Thread> threads = new ArrayList<>();
    final Map<Integer, MixpanelFlagVariant> results =
          ...
          new Thread(
              () -> {
                try {
                  ...
                  // Wait for callback
                  if (variantLatch.await(5000, TimeUnit.MILLISECONDS)) {
    ...
    // Flush loopers repeatedly to drive async processing
    for (int i = 0; i < 20; i++) {
      flushAllLoopers();
  ...
  public void testUninitializedTimingState() throws InterruptedException, JSONException {
    ...
    // Flush loopers for tracking callback
    flushAllLoopers();
