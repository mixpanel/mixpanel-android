 ...
 * Two guards are applied here instead — skip loopers whose thread is gone, and bound whatever is
 * left so a wedged looper costs seconds rather than the whole job.
 */
final class LooperTestUtils {
  ...
  private static final long IDLE_TIMEOUT_SECONDS = 10;
  ...
  private static void forEachLiveLooper(LooperAction action) {
    for (final Looper looper : ShadowLooper.getAllLoopers()) {
      ...
      try {
        ...
        pending.cancel(true);
        MPLog.w(LOGTAG, "Timed out idling looper " + looper + "; continuing", e);
      } catch (Exception e) {
